...
///
/// Not: Eski `world.openfoodfacts.org/cgi/search.pl` ve `/api/v2/search` uçları
/// aşırı yüklü olduğundan sık sık zaman aşımına uğruyordu (uygulamada "internet
/// hatası" olarak görünüyordu). Bunun yerine yeni, hızlı **Search-a-licious**
/// arama servisi kullanılıyor: `search.openfoodfacts.org/search` (yanıtı
/// `hits` listesi olarak döner, ~0.5 sn).
class OffService {
  static const _host = 'search.openfoodfacts.org';
  ...
  Future<List<Food>> search(String query) async {
    ...
    // Search-a-licious sonuçları `hits` altında döner (eski API `products`).
    final products =
        (data['hits'] as List?) ?? (data['products'] as List?) ?? const [];
